...
def compute_dual_rate_report(
    ...
    string of the form ``"{shares} shares"`` (e.g. ``"5.235 shares"``).
    The reporter renders this via ``_qty_from_description()`` in the Qty column,
    matching the RSU table layout.
    ...
    for event in sorted(stock.espp_events, key=lambda e: e.purchase_date):
        ...
        description = f"{event.shares_purchased} shares"
        espp_rows.append(
            DualRateEventRow(
